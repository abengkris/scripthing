...
describe('Script Service', () => {
  const prismaMock = {
    script: {
      ...
      update: vi.fn().mockImplementation(({ data }) => Promise.resolve({ id: 's1', ...data })),
  ...
  it('should create a script', async () => {
    ...
    const result = await scriptService.createScript(prismaMock, 'p1', data);
    expect(result.title).toBe('New Script');
    expect(result.projectId).toBe('p1');
  });

  it('should list scripts', async () => {
    const result = await scriptService.listScripts(prismaMock, 'p1');
    expect(result).toHaveLength(1);
  });

  it('should get a script', async () => {
    const result = await scriptService.getScript(prismaMock, 's1');
    expect(result.id).toBe('s1');
  });

  it('should update a script', async () => {
    const result = await scriptService.updateScript(prismaMock, 's1', { title: 'Updated' });
    expect(result.title).toBe('Updated');
  });

  it('should delete a script', async () => {
    const result = await scriptService.deleteScript(prismaMock, 's1');
    expect(result.id).toBe('s1');
  });
});
